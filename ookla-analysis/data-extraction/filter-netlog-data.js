...
const processHttpStreamJobIds = () => {
    ...
    try {
        ...
        if (fs.existsSync(uploadDirPath)) {
            console.log(`\nNote: httpStreamJobIds.txt already exists in ${directory}`);
        }

        writeFileSync(path.join(directory, 'httpStreamJobIds.txt'), http_stream_job_ids.join('\n'));
    }
};
...
const processSocketIds = () => {
    ...
    try {
        ...
        writeFileSync(path.join(directory, 'socketIds.txt'), socketIds.join('\n'));
    }
};
