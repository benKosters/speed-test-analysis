...
function calculateTimeDifference(currentTimestamp, previousTimestamp) {
    if (!previousTimestamp) return 0;
    const current = new Date(currentTimestamp);
    const previous = new Date(previousTimestamp);
    return (current - previous) / 1000; // Convert milliseconds to seconds
}
...
if (require.main === module) {
    ...
    if (args.length === 0) {
        console.log('Usage: node filter-cpu-data.js <input-json-file> [output-csv-file]');
        console.log('');
        console.log('Example:');
        console.log('  node filter-cpu-data.js cpu_metrics.json');
        console.log('  node filter-cpu-data.js cpu_metrics.json output.csv');
        process.exit(1);
    }
    ...
    filterCPUData(inputFile, outputFile);
}
