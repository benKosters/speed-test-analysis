...
NETLOG_FILTER_SCRIPT="${SCRIPT_DIR}/ookla/netlog-filter/main.js"
...
ANALYSIS_SCRIPT="${SCRIPT_DIR}/data-analysis/single-test-analysis/main.py"
...
if [ "$FILTER_MODE" != "-upload" ]; then
    if [ -d "$DOWNLOAD_DIR" ] && [ -f "$DOWNLOAD_DIR/byte_time_list.json" ]; then
        echo -e "\n========= Step 2: Analyzing download data ========="
        ...
        echo -e "\n========= Skipping download analysis - no download data found ========="
    fi
fi
...
if [ "$FILTER_MODE" != "-download" ]; then
    if [ -d "$UPLOAD_DIR" ] && [ -f "$UPLOAD_DIR/current_position_list.json" ]; then
        ...
        echo -e "\n========= Skipping upload analysis - no upload data found ========="
    fi
fi
