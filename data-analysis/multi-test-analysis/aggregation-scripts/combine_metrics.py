...
This script can combine two types of data:
...
This script was made with the assistance of AI due to limited time.
"""
...
class TestAggregator:
    ...
    def combine_core_data(self, test_dir: Path) -> bool:
    ...
    def combine_configuration_data(self, test_dir: Path, test_number: int) -> bool:
    ...
    def process_test_directory(self, test_dir: Path, test_number: int):
        ...
        if self.data_mode in ['core', 'both']:
            if not self.combine_core_data(test_dir):
                success = False
                errors.append("Failed to combine core data")

        # Aggregate configuration data
        if self.data_mode in ['configuration', 'both']:
            if not self.combine_configuration_data(test_dir, test_number):
                success = False
                errors.append("Failed to combine configuration data")
    ...
    def save_results(self, test_dir: Path):
        ...
        if self.data_mode in ['core', 'both'] and self.core_data:
            ...
            output_path = self.output_dir / "combined_core_data.csv"
        ...
        if self.data_mode in ['configuration', 'both'] and self.configuration_data:
            ...
            if len(self.unique_servers) == 1:
                ...
                filename = f"{server_name}_combined_configuration_data.csv"
            else:
                filename = "combined_configuration_data.csv"
...
Examples:
  ...
  python combine_metrics.py -t /path/to/tests -o ./output -f multi

  # Aggregate only core data from download tests
  python combine_metrics.py -t /path/to/tests -o ./output -m core -p download

  # Aggregate only configuration data from single-flow tests
  python combine_metrics.py -t /path/to/tests -o ./output -m configuration -f single

  # Aggregate both types from a specific batch
  python combine_metrics.py -t /path/to/batch_folder -o ./csvs -m both
        """
    )
    ...
    parser.add_argument(
        '-o', '--output-dir',
        required=True,
        help='Output directory to save combined CSV files'
    )
    ...
    parser.add_argument(
        ...
        default='both',
        help='Type of data to combine (default: both)'
    )
