...
def plot_slowstart(folder, df, threshold):
    """Plot throughput over time with the detected slow start threshold."""
    t0 = df["time"].iloc[0]
...
def load_bytecount_json(folder: str):
    """Load byte count data from a JSON file and convert it to a DataFrame."""
...
def bytecount_to_throughput(df):
    """Convert byte count data to throughput (Mbps) over time."""
    df = df.copy()
...
def detect_slow_start(timestamps, throughput,
                             ...
                             consecutive=3,
                             min_samples=5):
    """Detect slow start artifacts in throughput data using a growth ratio method.
    Args:
        timestamps: Array of timestamps corresponding to throughput measurements.
        throughput: Array of throughput measurements.
        growth_threshold: Minimum growth ratio to consider as non-slow start.
        consecutive: Number of consecutive violations required to confirm slow start.
        min_samples: Minimum number of samples before starting detection.
    Returns:
        Timestamp indicating the end of the slow start phase."""
