...
def run_throughput_calculation_driver(byte_count, aggregated_time, begin_time, bin_size, data_selection, stats_accumulator, config_accumulator):
    """
        Main driver to compute the throughput. For this project, we attempted to use two methods for throughput computation:

        1) Bin size as a minimum threshold
            Does not break apart original bytecount events. We only combine them if the interval of time is under the bin size. The interval of time can be greater than or
            equal to the bin size. For example, if the bin size is 5ms: if two events have intervals of 3ms each, they will be combined to one interval of 6ms.

            **NOTE** this method is NO LONGER USED, and WAS NOT USED IN THE PAPER SUBMISSION

        2) Bin size as a strict threshold
            Break apart byte count events so all bytecount events are exactly the size of the bin size.

        The throughput functions are defined in throughput_calculation.py, and
    """
    ...
    # Calculate throughput using strict intervals (the bin size)
    ...
    # calculate throughput using interval thresholds (intervals can be greater than or equal to the bin size)
    threshold_interval_throughput_results, threshold_interval_discarded_stats = tp_calc.calculate_interval_threshold_throughput(
        aggregated_time, byte_count, num_flows, bin_size, begin_time, data_selection)
    ...
    # These are NOT added to config_accumulator since they're just for visualization - they still need to be returned though
    threshold_throughput_by_flows = {}
    strict_throughput_by_flows = {}
    for flow_count in range(1, num_flows + 1):
        threshold_throughput_by_flows[flow_count], _ = tp_calc.calculate_interval_threshold_throughput(
