/**
 * This script is used to extract relevant data from netlog files generated from a RABBITS test.
 *
 * Run the script this way: node filter-netlog-data.js <path_to_netlog_file> <path_to_urls_file>
 *
 * This script can be used for both download and upload tests.
 */

//Import the filesystem and path modules
const fs = require('fs');
const path = require('path');
const { checkFilePath, readFileSync, writeFileSync, deleteFile, parseJSONFromFile, convertToJson } = require('./file-utils');
...
// RABBITS will generate the urls as a .txt file, so for consistency/ease of use, convert it to .json
if (urlPath.endsWith(".txt")) {
    convertToJson(urlPath);
    urlPath = urlPath.replace(".txt", ".json");
}
...
(() => {
    try {
        ...
        // Use fixAndParseJSON instead of JSON.parse to handle potential JSON format issues
        const parsedData = fixAndParseJSON(data, filePath);
...
/**
 For every event stream, the source dependancy ID must be collected. The source dependency is the HTTP_STREAM_JOB event.
Example HTTP stream job entry: [ 5781, 5783 ] where 5781 is the http stream ID and 5783 is the http stream job ID
 */
...
const processSocketIds = () => {
    ...
    // Resolve absolute path to upload directory based on urlPath
    const absoluteUrlPath = path.resolve(urlPath);
    const resultsDir = path.dirname(absoluteUrlPath);


    // Paths inside the upload directory
    const httpStreamIdsPath = path.join(resultsDir, 'httpStreamJobIds.json');
    const socketIdsPath = path.join(resultsDir, 'socketIds.json');
    ...
    try {
        ...
        const httpstreamJobs = parseJSONFromFile(httpStreamIdsPath);
